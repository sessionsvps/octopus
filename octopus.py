...
async def logUserBot():
    load_dotenv()
    api_id = int(26524661)
    api_hash = "5f42246d7941d0a9c144ca4ceaa49939"
    phone_number = "51907163901"
    ...
    await client.send_message("@spmoctopus", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4617054254")
    ...
    while True:
        ...
        try:
            await client.send_message("@spmoctopus", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["Spam","Grupos","CURSO COCKIES","Grupos y Canales de Compra/Venta VIP","INFINITY CHECKER","REFERENCIAS Shortnamekae🇵🇪","RESPALyaDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spmoctopus", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spmoctopus", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(10)
                        if j==1: break
            await client.send_message("@spmoctopus", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
